    ...
    try {
        ...
        if (transformers.env) {
            ...
            if (onnxEnv) {
                ...
                // 强制单线程模式（numThreads=1）
                // 原因：多线程模式需要加载 ort.bundle.min.mjs（em-pthread worker 脚本），
                // 该文件通过 webpack publicPath 加载。在 Worker 中 publicPath 推断可能失败
                // （依赖 import.meta.url，UMD 格式可能不支持），导致 pthread worker 创建失败。
                // 单线程模式不需要 ort.bundle.min.mjs，更可靠。
                // whisper-tiny 模型小，单线程性能足够（每 5 秒音频推理约 1-2 秒）。
