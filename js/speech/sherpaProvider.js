...
// ========== 资源 URL ==========
// JS 包装层从 jsDelivr 加载（speech-asr npm 包，包含编译好的 WASM）
// jsdelivr 在中国大陆可达且支持 CORS，避免 huggingface.co 被墙的问题
// speech-asr 包的 API 与 k2-fsa 原始仓库完全一致（OfflineRecognizer/createVad/CircularBuffer）
const SHERPA_ASR_JS_URL = 'https://cdn.jsdelivr.net/npm/speech-asr@1.1.6/dist/lib/sherpa-onnx-asr.js';
const SHERPA_VAD_JS_URL = 'https://cdn.jsdelivr.net/npm/speech-asr@1.1.6/dist/lib/sherpa-onnx-vad.js';

// Emscripten JS glue + WASM 二进制从 jsdelivr 加载（speech-asr 包）
// 注意：文件名是 sherpa-onnx-wasm-main-asr（不含 vad），但功能完整（VAD + ASR）
const SHERPA_WASM_GLUE_URL = 'https://cdn.jsdelivr.net/npm/speech-asr@1.1.6/dist/lib/sherpa-onnx-wasm-main-asr.js';
const SHERPA_WASM_BASE = 'https://cdn.jsdelivr.net/npm/speech-asr@1.1.6/dist/lib';

// 自动检测同源代理路径（Cloudflare Pages Functions 部署在 /api/proxy）
const _SHERPA_PROXY_BASE = window.location.pathname.replace(/[^/]*$/, '').replace(/\/$/, '') + '/api/proxy?url=';

// 模型文件通过 CORS 代理从 HuggingFace 模型仓库加载
// huggingface.co 在中国大陆被墙，通过 /api/proxy 代理（Cloudflare Worker 全球节点可达）
// SenseVoice-Small int8 量化版（约229MB）+ tokens.txt + Silero VAD
const SHERPA_MODEL_ORIGIN_BASE = 'https://huggingface.co/csukuangfj/sherpa-onnx-sense-voice-zh-en-ja-ko-yue-2024-07-17/resolve/main';
const SHERPA_MODEL_FILES = {
    'sense-voice.onnx': `${_SHERPA_PROXY_BASE}${encodeURIComponent(SHERPA_MODEL_ORIGIN_BASE + '/model.int8.onnx')}`,
    'tokens.txt': `${_SHERPA_PROXY_BASE}${encodeURIComponent(SHERPA_MODEL_ORIGIN_BASE + '/tokens.txt')}`,
    // silero_vad.onnx 在 GitHub releases，也通过代理
    'silero_vad.onnx': `${_SHERPA_PROXY_BASE}${encodeURIComponent('https://github.com/k2-fsa/sherpa-onnx/releases/download/asr-models/silero_vad.onnx')}`,
};
...
class SherpaProvider extends (SpeechProvider || class {}) {
    ...
    _loadScript(url) {
        return new Promise((resolve, reject) => {
            ...
            script.src = url;
            script.async = true;
            // COEP=credentialless 环境下，跨域 script 必须设置 crossorigin 属性
            // jsdelivr 支持 CORS，设置 crossorigin="anonymous" 会触发 CORS 请求
    ...
    _initWasm() {
        return new Promise(async (resolve, reject) => {
            ...
            window.Module = {
                ...
                // .wasm 文件从 jsdelivr 加载（支持 CORS，中国大陆可达）
