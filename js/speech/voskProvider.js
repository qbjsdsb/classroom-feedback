...
//
// ⚠️ CORS 问题：alphacephei.com 不发送 Access-Control-Allow-Origin 头，
// 浏览器跨域 fetch 会被拦截。通过 Cloudflare Pages Functions 代理（/api/proxy）添加 CORS 头。
// 代理仅转发白名单域名，不会被滥用为开放代理。
const VOSK_MODEL_ORIGIN = 'https://alphacephei.com/vosk/models/vosk-model-small-cn-0.22.zip';
// 自动检测同源代理路径（Cloudflare Pages Functions 部署在 /api/proxy）
const _VOSK_BASE_PATH = window.location.pathname.replace(/[^/]*$/, '').replace(/\/$/, '');
const VOSK_MODEL_URL = `${_VOSK_BASE_PATH}/api/proxy?url=${encodeURIComponent(VOSK_MODEL_ORIGIN)}`;
