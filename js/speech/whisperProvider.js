...
class WhisperProvider extends (SpeechProvider || class {}) {
    ...
    async preload(onProgress) {
        ...
        try {
            ...
            // ONNX Runtime WASM 文件从同源 /vendor/ 加载（已包含 ort-wasm-simd-threaded.jsep.wasm）
            // 同源加载避免 COEP=credentialless 环境下跨域 fetch 的潜在问题
            const _wasmPaths = _basePath + '/vendor/';
            ...
            try {
                ...
                // 额外检查：ONNX 模型文件是否真正存在
                // Cloudflare Pages 单文件 25MB 限制，30MB 的 .onnx 不会部署
                // 但 config.json 等小文件会被部署，导致探测误判本地模型可用
                // SPA 兜底页对 .onnx 请求返回 200 + text/html，需检查 content-type
                const onnxProbe = await fetch(_localModelPath + '/onnx/decoder_model_merged_quantized.onnx', { method: 'HEAD' });
                const onnxType = onnxProbe.headers.get('content-type') || '';
                if (!onnxProbe.ok || onnxType.includes('text/html') || onnxType.includes('application/json')) {
                    throw new Error('ONNX 模型文件不可用（被 SPA 兜底）');
                }
                console.log('[Whisper] 本地模型可用，使用本地模式，model_type=', config.model_type);
            } catch (probeErr) {
                console.warn('[Whisper] 本地模型不可用，切换到远程模式（同源代理→hf-mirror.com）:', probeErr.message);
                _localFilesOnly = false;
                _modelPath = 'Xenova/whisper-tiny';  // HuggingFace 上的 whisper-tiny 模型 ID
                // 用同源代理 /api/hf/ 转发到 hf-mirror.com，避免：
                // 1. hf-mirror.com 307 重定向可能导致的 CORS 问题
                // 2. COEP=credentialless 环境下跨域 fetch 的潜在限制
                // transformers.js URL 拼接：remoteHost + "{model}/resolve/{revision}/" + fileName
                // 结果：/api/hf/Xenova/whisper-tiny/resolve/main/config.json（同源请求）
                _remoteHost = window.location.origin + _basePath + '/api/hf/';
